...
exports.handlers = new Map([
    ...
    [
        ...
        async function (runState, common) {
            ...
            const currentStorage = (0, util_1.setLengthLeftStorage)(await runState.eei.storageLoad(keyBuf));
            if (common.hardfork() === 'constantinople' || common.gteHardfork('istanbul')) {
                const originalStorage = (0, util_1.setLengthLeftStorage)(await runState.eei.storageLoad(keyBuf, true));
    ...
    [
        ...
        async function (runState, common) {
            ...
            const gasLimit = (0, util_1.maxCallGas)(currentGasLimit, runState.eei.getGasLeft(), runState, common);
            ...
            if (!value.isZero()) {
                const callStipend = new ethereumjs_util_1.BN(common.param('gasPrices', 'callStipend'));
    ...
    [
        ...
        async function (runState, common) {
            ...
            (0, util_1.subMemUsage)(runState, outOffset, outLength, common);
            (0, EIP2929_1.accessAddressEIP2929)(runState, toAddress, common);
            const gasLimit = (0, util_1.maxCallGas)(currentGasLimit, runState.eei.getGasLeft(), runState, common); // we set TangerineWhistle or later to true here, as STATICCALL was available from Byzantium (which is after TangerineWhistle)
            let data = Buffer.alloc(0);
            if (!inLength.isZero()) {
                data = runState.memory.read(inOffset.toNumber(), inLength.toNumber());
            }
