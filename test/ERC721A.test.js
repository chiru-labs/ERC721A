  ...
  context('burn', async function () {
    ...
    describe('zero-indexed', function () {
      beforeEach(async function () {
        ...
        this.owner = owner;
        this.addr1 = addr1;
        await this.erc721a['safeMint(address,uint256)'](this.addr1.address, 10);
        await this.erc721a.connect(this.addr1).burn(5);
      });

      it('changes exists', async function () {
        expect(await this.erc721a.exists(5)).to.be.false;
      })

      it('cannot burn a non-existing token', async function () {
        await expect(this.erc721a.connect(this.addr1).burn(10)).to.be.revertedWith(
          'ERC721A: owner query for nonexistent token'
        )
      })

      it('cannot burn a token twice', async function () {
        await expect(this.erc721a.connect(this.addr1).burn(5)).to.be.revertedWith(
          'ERC721A: owner query for nonexistent token'
        )
      })

      it('reduces totalSupply', async function () {
        const supplyBefore = await this.erc721a.totalSupply();
        await this.erc721a.connect(this.addr1).burn(2);
        expect(supplyBefore - (await this.erc721a.totalSupply())).to.equal(1);
        await this.erc721a.connect(this.addr1).burn(1);
        expect(supplyBefore - (await this.erc721a.totalSupply())).to.equal(2);
      })

      it('adjusts owners tokens by index', async function () {
        const n = await this.erc721a.totalSupply();
        for (let i = 0; i < 5; ++i) {
          expect(await this.erc721a.tokenByIndex(i)).to.be.equal(i);
        }
        for (let i = 5; i < n; ++i) {
          expect(await this.erc721a.tokenByIndex(i)).to.be.equal(i+1);
        }
        expect(await this.erc721a.tokenOfOwnerByIndex(this.addr1.address, 2)).to.be.equal(2);
        await this.erc721a.connect(this.addr1).burn(2);
        expect(await this.erc721a.tokenOfOwnerByIndex(this.addr1.address, 2)).to.be.equal(3);
        await this.erc721a.connect(this.addr1).burn(0);
        expect(await this.erc721a.tokenOfOwnerByIndex(this.addr1.address, 2)).to.be.equal(4);
      })

      it('adjusts owners balances', async function () {
        expect(await this.erc721a.balanceOf(this.addr1.address)).to.be.equal(9);
      });
      ...
      it('adjusts token by index', async function () {
        ...
        await expect(this.erc721a.tokenByIndex(n)).to.be.revertedWith(
    ...
    describe('one-indexed', function () {
      beforeEach(async function () {
        ...
        this.owner = owner;
        this.addr1 = addr1;
        await this.erc721a.initOneIndexed();
        await this.erc721a['safeMint(address,uint256)'](this.addr1.address, 10);
        await this.erc721a.connect(this.addr1).burn(5+1);
      });

      it('changes exists', async function () {
        expect(await this.erc721a.exists(5+1)).to.be.false;
      })

      it('has correct last index', async function () {
        await this.erc721a.connect(this.addr1).burn(10);
      })

      it('cannot burn a non-existing token', async function () {
        await expect(this.erc721a.connect(this.addr1).burn(11)).to.be.revertedWith(
          'ERC721A: owner query for nonexistent token'
        )
      })

      it('cannot burn a token twice', async function () {
        await expect(this.erc721a.connect(this.addr1).burn(5+1)).to.be.revertedWith(
          'ERC721A: owner query for nonexistent token'
        )
      })
      ...
      it('adjusts owners tokens by index', async function () {
        ...
        for (let i = 5; i < n; ++i) {
          expect(await this.erc721a.tokenByIndex(i)).to.be.equal(i+2);
        }
        expect(await this.erc721a.tokenOfOwnerByIndex(this.addr1.address, 2)).to.be.equal(2+1);
        await this.erc721a.connect(this.addr1).burn(2+1);
        expect(await this.erc721a.tokenOfOwnerByIndex(this.addr1.address, 2)).to.be.equal(3+1);
        await this.erc721a.connect(this.addr1).burn(0+1);
        expect(await this.erc721a.tokenOfOwnerByIndex(this.addr1.address, 2)).to.be.equal(4+1);
      })
