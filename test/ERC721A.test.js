    ...
    describe('tokensOfOwner', async function() {
      it('returns the correct token ids', async function() {
        ...
        for(const expected_result of expected_results) {
          const bn_tokens = await this.erc721a['tokensOfOwner(address)'](expected_result.owner.address);
      ...
      it('returns the correct token ids after a transfer interferes with the normal logic', async function() {
        // Owner of 6,7,8
        await this.erc721a['safeMint(address,uint256)'](this.owner.address, 3);

        // Break sequential order
        await this.erc721a['transferFrom(address,address,uint256)'](this.owner.address, this.addr4.address, 7);

        // Load balances
        const owner_bn_tokens = await this.erc721a['tokensOfOwner(address)'](this.owner.address);
        const addr4_bn_tokens = await this.erc721a['tokensOfOwner(address)'](this.addr4.address);
